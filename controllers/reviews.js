...
async function upvoteReview(req, res, next) {
    ...
    try {
        ...
        if (review.upVote.includes(req.user._id)) {
            review.upVote = review.upVote.filter((userId) => !userId.equals(req.user._id));
        } else {
            review.upVote.push(req.user._id);
        }
        review.downVote = review.downVote.filter((userId) => !userId.equals(req.user._id));
...
module.exports = { getReviewOfUser, getReviews, addReviews, updateReview, deleteReview, upvoteReview, downvoteReview }